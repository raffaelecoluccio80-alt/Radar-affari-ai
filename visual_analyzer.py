...
def _extract_output_text(payload: Dict[str, Any]) -> str:
    ...
    raise RuntimeError("La risposta OpenAI non contiene output testuale leggibile.")


async def analyze_listing(url: str) -> Dict[str, Any]:
    if not OPENAI_API_KEY:
        raise RuntimeError("OPENAI_API_KEY non configurata nelle variabili Railway.")

    listing = await fetch_listing_context(url)
    image_urls = listing["image_urls"]
    if not image_urls:
        raise RuntimeError("Nessuna immagine dell'annuncio trovata.")
    ...
    if response.status_code >= 400:
        raise RuntimeError(
            f"OpenAI API HTTP {response.status_code}: {response.text[:800]}"
        )
